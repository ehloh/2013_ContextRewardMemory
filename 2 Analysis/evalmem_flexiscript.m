...
where.where='/Users/EleanorL/Dropbox/sandisk/9 Deacon project/3 Analysis';
where.data=[where.where filesep '2 Data new ELacquired/v3_5'];
% where.data=[where.where filesep '2 Data/v3.5'];
where.scripts=[where.where filesep '1 Analysis scripts'];
where.printfiles=[where.where filesep '3 Analysis inputs'];
...
for s=1:size(w.dir,1) % Split into cell-samples (according to v3.4)
    ws.ts=memdata{s,1};
    ws.foils=ws.ts(ws.ts(:,3)==2,:); 
    
%     disp('Dummy Sim & Dis values!')
    ws.ts(:,col.simordis)=1;
    ...
    ws.s1_1=ws.s1(ws.s1(:,col.seedtrialtype)==1,:); % Seed type is marked 1-4: (1=[00],2=[+0],2.5=[0+],3=[++])
    ws.s1_2=ws.s1(ws.s1(:,col.seedtrialtype)==2,:);
    ws.s1_3=ws.s1(ws.s1(:,col.seedtrialtype)==3,:);
    ws.s1_4=ws.s1(ws.s1(:,col.seedtrialtype)==4,:);
    ws.s2_1=ws.s2(ws.s2(:,col.seedtrialtype)==1,:);
    ws.s2_2=ws.s2(ws.s2(:,col.seedtrialtype)==2,:);
    ws.s2_3=ws.s2(ws.s2(:,col.seedtrialtype)==3,:);
    ws.s2_4=ws.s2(ws.s2(:,col.seedtrialtype)==4,:);
    ws.s3_1=ws.s3(ws.s3(:,col.seedtrialtype)==1,:);
    ws.s3_2=ws.s3(ws.s3(:,col.seedtrialtype)==2,:);
    ws.s3_3=ws.s3(ws.s3(:,col.seedtrialtype)==3,:);
    ws.s3_4=ws.s3(ws.s3(:,col.seedtrialtype)==4,:);
    ws.s4_1=ws.s4(ws.s4(:,col.seedtrialtype)==1,:);
    ws.s4_2=ws.s4(ws.s4(:,col.seedtrialtype)==2,:);
    ws.s4_3=ws.s4(ws.s4(:,col.seedtrialtype)==3,:);
    ws.s4_4=ws.s4(ws.s4(:,col.seedtrialtype)==4,:);
...
% Mean scores
m_assocpercentrec=cell(size(w.dir,1), size(instruc,1));
...
ws.s0=[]; % dummy
    for i=1:size(instruc,1) % Excute entire instruction list
        ...
        if size(wc.sample,1)>0
            ...
            m_assocpercentrec{s,i}=wc.nhits/wc.nrecold;
        else
            error
            m_assoc{s,i}=[];
            m_assocpercentrec{s,i}=[];
        end
    end
end
...
ws.s0=[]; % dummy
    for i=1:size(instruc_diff,1) % Excute entire instruction list
        ...
        if size(wc.psample,1)>0 && size(wc.msample,1)>0
            ...
            % Output to group matrix
            d_assocpercentrec{s,i}=wc.p_nhits/wc.p_nrecold-wc.m_nhits/wc.m_nrecold;
            d_assoc{s,i}=wc.p_nhits/size(wc.psample,1)-wc.m_nhits/size(wc.msample,1);
        else
            d_assocpercentrec{s,i}=[];
            d_assoc{s,i}=[];
        end
    end
end
...
%     disp([measures{m} ': File print=' num2str(printok.fileprint) ', File moved=' num2str(printok.filemoved)])
    disp('Disabled printing')
end


assoc=[headers;  [subj m_assoc d_assoc]];
assocpercentrec=[headers;  [subj m_assocpercentrec d_assocpercentrec]];

openvar m_assoc
openvar assocpercentrec
