...
w.testing=1;
where=pwd; % '/Volumes/MEDIASTAR/6 [v3 Seeds] Experiment MDeacon/2 v3.2 Experiment execution';
...
elseif w.testing==1 % testing
    ...
    w.screenmode=1;
%     p.itemcounterbal=input('Item counterbalancing version (1-2) : ');
end
p.itemcounterbal=input('Item counterbalancing version (1-2) : ');
w.res=3;
try
...
w.subject.filename=strcat(w.subjname,'_learning');
p.subjectlog.name=w.subjname;
p.n.scenetypes=2;
p.contingency_high=0.9;
p.n.ntrials=64;
p.n.contexts=4;
if round(p.n.ntrials/p.n.contexts)~=p.n.ntrials/p.n.contexts
    input('Error: Number of trials must be a multiple of 4!')
end
p.n.dummyitems=20; 
%
p.rewardpertrial=20;
...
p.resp.item_natural=98;
p.resp.item_manmade=97;
w.line=50; 
w.line1=40;
end
for o1=1:1 % PARAMETERS: Subject-specific
load(['Data' filesep w.subjname '_file_0thresholding.mat'])
p.RThit=thresholding.settings.itemcutoff;
...
for j=1:p.n.contexts
    ...
    ws.t(:,2)=round((in.contexts(j)-ws.t(:,1))*10);
    ws.t(:,3)=ws.t(:,1);
...
w.item1=[];
w.item2=[];
for i=1:size(learning_itemlist,1)
    if learning_itemlist{i,2}==1
        w.item1=vertcat(w.item1, learning_itemlist{i,3});
    else
        w.item2=vertcat(w.item2, learning_itemlist{i,3});
    end
end
w.count1=1;
w.count2=1;
...
% Note: Instrctions are for Item x Context =reward. BUT these instructions
% are never shown!
for o1=1:1
     if w.testing==111 
...
for trialnum=1:p.n.ntrials
    ...
    for o4=1:1 % Set up for current trial
        itemfilename=learning_itemlist{data(trialnum,8),1};
    ...
    cgtext('Natural',230,-345)
    cgtext('Man-made',-230,-345)
    waituntil(w.fixationonset*1000+p.disp.framelength_learn/2+randi(p.disp.framelength_learn/2))
    clearkeys
...
%% 
        ...
        cgtext('You have 2 more stage to go', 0, w.line*-1)
...
% Recalculate RT threshold
w.gooddata=data(data(i,13)==1,12);
p.postlearning_RT=thresholding.settings.itemcutoff;
...
learning.results_tasks=res;
w.savefile=strcat(p.subjectlog.name,'_file_1learning'); 
w.savefilecommand=strcat('save(w.savefile, ''learning'')');
